...
class WoTReplayParser{
	...
	public function parse($filename,$wotreplays=false){
		...
		if($wotreplays){
			$this->printDebug("Processing wotreplays");
			$this->fp=$filename;
		}
		else{
			...
			while($currentBlockIndex <= $blockCount){
				try{
					...
					if($currentBlockIndex == 1){
						...
						if(array_key_exists('battleType', $decodedBlockData)){ 
					...
					if(($personalInfo = $this->array_get_key_recursive("personal", $decodedBlockData)) !== false){
						...
						if(!isset($replayParserMethod)){
							$replayVersionString=$this->getVersionFromDate($decodedBlockData['common']["arenaCreateTime"]);
	...
	private function getVersionFromDate($time){
		$this->printDebug('Определяем версию реплея');
		switch(true){
			case ($time > 1371556800):
			return '0.8.6.0';
			break;
			case ($time > 1366286400 and $time < 1371556800):
			return '0.8.5.0';
			break;
			case ($time > 1362052800 and $time < 1366286400):
			return '0.8.4.0';
			break;
			case ($time > 1358251200 and $time < 1362052800):
			return '0.8.3.0';
			break;
			case ($time > 1354795200 and $time < 1358251200):
			return '0.8.2.0';
			break;
			case ($time > 1351166400 and $time < 1354795200):
			return '0.8.1.0';
			break;
			default:
			return '0.8.0.0';
	...
	private function clear_array_for_key($ar,$key){
		foreach($ar as $k => $massiv){		
			...
			if(array_key_exists($key, $arrOut)){
				$this->printDebug($key.' Ключик имеется');
				$new_aray[$arrOut[''.$key.'']]=$arrOut;
			} else{
				return $this->error('Ошибка в блоке players','2');
	...
	*/
	private function error($err,$error_code=1){
		if($this->debug){
			$this->printDebug("<br>Error: $err");
		}
		$this->addResult("error.message", $err);
		$this->addResult("error.type", $error_code);
		return $this->renderJson();
	}
